...
class CashFlowController extends Controller
{
    ...
    public function update(Request $request, int $year): RedirectResponse
    {
        ...
        $payload = $request->validate([
            ...
            'incomes.*.isRecurring' => ['nullable', 'boolean'],
            'incomes.*.recurrenceInterval' => ['nullable', 'integer', 'min:1', 'max:12'],
            'incomes.*.recurrenceStartMonth' => ['nullable', 'integer', 'min:1', 'max:12'],
            'incomes.*.recurrenceEndMonth' => ['nullable', 'integer', 'min:1', 'max:12'],
            ...
            'expenses.*.isRecurring' => ['nullable', 'boolean'],
            'expenses.*.recurrenceInterval' => ['nullable', 'integer', 'min:1', 'max:12'],
            'expenses.*.recurrenceStartMonth' => ['nullable', 'integer', 'min:1', 'max:12'],
            'expenses.*.recurrenceEndMonth' => ['nullable', 'integer', 'min:1', 'max:12'],
    ...
    private function mapRows($rows, bool $withClient = false): array
    {
        return $rows->sortBy('sort_order')->values()->map(function (CashFlowRow $row) use ($withClient) {
            ...
            $entry = [
                ...
                'recurrenceStartMonth' => $row->recurrence_start_month,
                'recurrenceEndMonth' => $row->recurrence_end_month,
                'monthly' => $monthly,
            ];
            if ($withClient) {
